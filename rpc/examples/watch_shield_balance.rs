...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let vkey = std::env::args().nth(1).expect("usage: watch_shield_balance <viewing-key>");
    ...
    let imported = client.import_sapling_viewing_key(&vkey, Some("whenkeyisnew"), None).await?;
    println!("watching {}", imported.address);

    let mut watcher = ShieldWatcher::new(
        &client,
        WatchOptions { addresses: vec![imported.address], ..Default::default() },
    );
    loop {
        for event in watcher.poll().await? {
            match event {
                ShieldEvent::Note(n) => println!("+{} PIV in {}:{}", n.amount, n.txid, n.outindex),
                ShieldEvent::Spent(n) => println!("-{} PIV ({}:{} spent)", n.amount, n.txid, n.outindex),
